...
interface EmptyStateProps {
  icon?: 'music' | 'artist' | 'album' | 'concert' | 'live' | 'search' | 'heart' | 'ticket';
  title: string;
  description: string;
  actionLabel?: string;
  actionHref?: string;
...
const icons: Record<string, React.ReactNode> = {
  ...
  concert: (
    <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
  ),
  live: (
...
export default function EmptyState({ icon = 'music', title, description, actionLabel, actionHref }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      <div className="text-dark-300 mb-4">{icons[icon]}</div>
      <h3 className="text-lg font-semibold text-white mb-2">{title}</h3>
      <p className="text-dark-500 text-sm max-w-sm mb-6">{description}</p>
      {actionLabel && actionHref && (
        <Link href={actionHref} className="bg-primary-500 hover:bg-primary-600 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-colors">
          {actionLabel}
        </Link>
      )}
    </div>
